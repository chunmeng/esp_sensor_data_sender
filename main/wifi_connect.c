...
static const char *TAG = "wifi_connect"; /* debug tag */
...
static void start(const char *ssid, const char *passwd)
{
    ...
    /* Register handlers to handle disconnection and connection */
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, WIFI_EVENT_STA_DISCONNECTED, &on_wifi_disconnect, NULL));
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &on_got_ip, NULL));
    ESP_ERROR_CHECK(esp_wifi_set_storage(WIFI_STORAGE_RAM));

    /* Set the wifi connection config */
...
static void stop(void)
{
    ...
    ESP_ERROR_CHECK(esp_wifi_clear_default_wifi_driver_and_handlers(s_wifi_netif));
